    ...
    <div>
        ...
        {user && (
          <div onClick={() => router.push('/news')}
           className={`flex items-center mb-2 cursor-pointer group relative ${expand ? "gap-2 text-white/80 text-sm p-2.5 border border-white/20 rounded-lg hover:bg-white/10" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg justify-center"}`}>
              ...
              <div className={`absolute w-max ${!expand && "-right-28"} -top-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                  News & Articles
                  <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-1/2" : "left-4"} -bottom-1.5`}></div>
              </div>
              {expand && <span>News & Articles</span>}
          </div>
        )}
        ...
        {user && (
          <div onClick={() => router.push('/profile')}
           className={`flex items-center mb-2 cursor-pointer group relative ${expand ? "gap-2 text-white/80 text-sm p-2.5 border border-white/20 rounded-lg hover:bg-white/10" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg justify-center"}`}>
              <Image className={expand ? "w-5" : "w-6"} src={assets.profile_icon} alt='Edit Profile'/>
              <div className={`absolute w-max ${!expand && "-right-28"} -top-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                  Edit Profile
                  <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-1/2" : "left-4"} -bottom-1.5`}></div>
              </div>
              {expand && <span>Edit Profile</span>}
          </div>
        )}
